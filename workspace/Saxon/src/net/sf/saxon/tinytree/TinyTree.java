...
public final class TinyTree {
    ...
    protected Value[] typedValueArray = null;
    ...
    */
    ...
    public void setConfiguration(Configuration config) {
        this.config = config;
        allowTypedValueCache = config.isLicensedFeature(Configuration.LicenseFeature.SCHEMA_VALIDATION) && config.useTypedValueCache();
    ...
    private void ensureNodeCapacity(short kind) {
        if (nodeKind.length < numberOfNodes+1) {
            ...
            if (typedValueArray != null) {
                Value[] typedValueArray2 = new Value[k];
    ...
    private void ensureAttributeCapacity() {
        if (attParent.length < numberOfAttributes+1) {
            ...
            if (attTypedValue != null) {
                Value[] attTypedValue2 = new Value[k];
...
//        }
    ...
    */
        ...
        if (typedValueArray == null || typedValueArray[nodeNr] == null) {
            ...
            if (annotation == StandardNames.XS_UNTYPED_ATOMIC || annotation == StandardNames.XS_UNTYPED ||
                ...
                if (stype == null) {
                    ...
                    Value value = stype.atomize(element);
                    if (allowTypedValueCache) {
                        if (typedValueArray == null) {
                            typedValueArray = new Value[nodeKind.length];
                        }
    ...
    /**
     * Get the type value of an attribute node, given only the node number. This method avoids
     * materializing the attribute node if possible
    ...
    public Value getTypedValueOfAttribute(int nodeNr) throws XPathException {
        if (attTypedValue == null || attTypedValue[nodeNr] == null) {
            int annotation = getAttributeAnnotation(nodeNr) & NamePool.FP_MASK;
            if (annotation == StandardNames.XS_UNTYPED_ATOMIC) {
                return new UntypedAtomicValue(attValue[nodeNr]);
            } else if (annotation == StandardNames.XS_STRING) {
                return new StringValue(attValue[nodeNr]);
                ...
                if (stype == null) {
                    ...
                    TinyAttributeImpl att = new TinyAttributeImpl(this, nodeNr);
                    Value value = stype.atomize(att);
                    if (allowTypedValueCache) {
                        if (attTypedValue == null) {
                            attTypedValue = new Value[numberOfAttributes];
