...
public class QueryModule implements StaticContext {
    ...
    private DecimalFormatManager decimalFormatManager = null;   // used only in XQuery 1.1
    ...
    private void resetFunctionLibraries() {
        ...
        if (getLanguageVersion().equals("1.1")) {
            functionSet |= StandardFunction.XQUERY11;
        }
    ...
    public boolean mayImportModule(String namespace) {
        ...
        for (int i=0; i<importers.size(); i++) {
            if (!(importers.get(i)).mayImportModule(namespace)) {
                return false;
            }
        }
        return true;
...
//        if (!executable.isSchemaAware() && mode != Validation.STRIP) {
//            throw new IllegalArgumentException("Cannot set construction mode = preserve if query is not schema-aware");
//        }
    }
    ...
    public boolean importsNamespaceIndirectly(String uri) {
        ...
        for (Iterator it = iterateImportedNamespaces(); it.hasNext();) {
            ...
            List<QueryModule> list = executable.getQueryLibraryModules(moduleURI);
            for (Iterator<QueryModule> i2 = list.iterator(); i2.hasNext();) {
                QueryModule sqc = i2.next();
...
//        }
    ...
    public void declareVariable(VariableDeclaration var) throws XPathException {
        ...
        if (variables.get(key) != null) {
            VariableDeclaration old = variables.get(key);
            if (old == var) {
                // do nothing
            } else {
                String oldloc = "";
    ...
    public List<GlobalVariableDefinition> fixupGlobalVariables(SlotManager globalVariableMap) throws XPathException {
        List<GlobalVariableDefinition> varDefinitions = new ArrayList<GlobalVariableDefinition>(20);
    ...
    public Expression bindVariable(StructuredQName qName) throws XPathException {
        VariableDeclaration var = variables.get(qName);
        if (var == null) {
     ...
     * This method is for internal use only.
     *
     * @throws net.sf.saxon.trans.XPathException if a function call refers to a function that has
     *                                        not been declared
     */
    ...
    public void bindUnboundVariables() throws XPathException {
        for (Iterator<UndeclaredVariable> iter = undeclaredVariables.values().iterator(); iter.hasNext();) {
            UndeclaredVariable uv = iter.next();
...
//         }
     ...
     * @throws net.sf.saxon.trans.XPathException (with error code XPST0081)
    ...
    public NamespaceResolver getNamespaceResolver() {
        List<Integer> externalNamespaceCodes = null;
        NamespaceResolver externalResolver = userQueryContext.getExternalNamespaceResolver();
        if (externalResolver != null) {
        ...
        for (Iterator<String> iter = explicitPrologNamespaces.keySet().iterator(); iter.hasNext();) {
        ...
        if (externalNamespaceCodes != null) {
            for (int a = 0; a < externalNamespaceCodes.size(); a++) {
                nscodes[used++] = (externalNamespaceCodes.get(a)).intValue();
            }
        }
     ...
     * @return the language version (currently "1.0" or "1.1")
     * @since 9.2
     */
     ...
     * @throws net.sf.saxon.trans.XPathException (error XQST0036) if the namespace is not present in a schema
