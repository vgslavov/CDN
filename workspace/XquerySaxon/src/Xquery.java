...
//					+ ((decryptEnd - decryptStart) / 1000.0) + " secs");
		...
		String queryName = uuid.toString() + ".xq";
		FileWriter fWriter = null;
		BufferedWriter bWriter = null;
		File file = null;
		try {
			file = new File(queryName);
			file.createNewFile();
			fWriter = new FileWriter(queryName);
			bWriter = new BufferedWriter(fWriter);

			bWriter.write(xQuery);
			bWriter.close();
			fWriter.close();
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			//String[] basexcmd = { "/usr/bin/java -cp BaseX76.jar org.basex.BaseX query.bsx > " + fileName };
			//System.out.println(basexcmd);
			...
			*/
			...
			String[] args = { "/usr/bin/java", "-cp", "/usr/share/tomcat6/BaseX76.jar", "org.basex.BaseX", "-o", fileName, queryName };
			...
			try {
				Process p = pb.start();
				System.out.println("BaseX exit status: " + p.waitFor());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
